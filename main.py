...
count = 4 #количество цифр в загаданном слове
...
def viewsettings():
    print("Количество цифр в загаданном числе: ", count)
    print("Генерация числа компьютером: ", ai)
    print("Могут ли повторятся цифры в загаданном числе: ", repetition)
def settings():
    word = input("""Введите название параметра, который Вы желаете изменить:
    count - изменяет количество цифр в загаданном слове
    ai - генерирует число, компьютер или человек загадывает сам
    repeat - могут ли повторятся цифры в загаданном числе
    back - если хотите вернуться назад
    """)
    if word == "count":
        count = int(input("Введите количетсво цифр в загаданном слове: "))
    elif word == "ai":
...
def play():
    ...
    if ai:
        ...
        if len(number) != count:
            print("Количество цифр в загаданном числе не соответсвует параметрам!")
            return
        if repetition:
            for i in number:
                if number.count(i) > 1:
    ...
    while game:
        ...
        if len(numb) != len(number):
            print("Количество цифр в вводимом числе не равно количетсву цифр в загаданном числе!")
            continue
        cows = 0
        bulls = 0
        for i in numb:
        ...
        if bulls == count:
            ...
            print("Коровы: {0}. Быки: {1}".format(cows, bulls))

print("Быки-Коровы")
print("  ------------------------------  ")
print("Для выхода из программы нажмите Ctrl+C")
while True:
    word = input("""      -     Для вывода параметров введите viewsettings
      -     Для изменения параметров введите settings
